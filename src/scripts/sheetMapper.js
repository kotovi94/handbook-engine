...
export function mapCharacterToSheetSections(character) {
  ...
  return [
    sheetSection("Pagina 1 - Identidad", [
      sheetField("clase nivel", `${displayName(derived.classData) || "Pendiente"} ${derived.level}`, "La creacion esta fijada para nivel 5."),
    ...
    sheetSection("Pagina 1 - Combate y supervivencia", [
      ...
      sheetField("PUNTOS DE GOLPE actuales", derived.hitPointMaximum, "Al comenzar la sesion normalmente coincide con el maximo."),
      ...
      sheetField("TAMANO", derived.size, "Proviene de la especie."),
      ...
      sheetField("SALVACIONES CONTRA MUERTE", "Sin marcar", "Se llenan durante la sesion si el personaje cae a 0 PG."),
    ]),
    sheetSection("Pagina 1 - Atributos", Object.entries(derived.abilities).map(([ability, score]) =>
      sheetField(
    ...
    sheetSection("Pagina 1 - Salvaciones y habilidades", [
      sheetField("Tiradas de salvacion", derived.savingThrows.length ? displayList(derived.savingThrows) : "Pendiente", "Marca estas salvaciones como competentes."),
      ...
      sheetField("PERCEPCION PASIVA", derived.passivePerception, "Base 10 + modificador de Sabiduria, antes de competencias o bonos."),
    ]),
    sheetSection("Pagina 1 - Entrenamiento y competencias", [
      ...
      sheetField("entrenamiento con armaduras", derived.armorTraining.length ? displayList(derived.armorTraining) : "Ninguno", "Marca ligeras, medias, pesadas y escudos segun corresponda."),
    ]),
    sheetSection("Pagina 1 - Dotes", [
      sheetField("DOTES", derived.grantedFeats.length ? derived.grantedFeats.map(sheetText) : "Pendiente", "Copia aqui las dotes obtenidas por trasfondo u otras elecciones."),
      sheetField("MEJORA DE NIVEL 4", level4SheetText(character, derived), "Si elegiste atributos, ya estan sumados en las puntuaciones. Si elegiste dote, copiala tambien en Dotes."),
    ]),
    sheetSection("Pagina 1 - Rasgos de clase", [
      ...
      sheetField("Rasgos de subclase", formatProgression(derived.subclassFeaturesByLevel), "Copia tambien los rasgos de subclase disponibles."),
    ]),
    sheetSection("Pagina 1 - Atributos de especie", [
    ...
    sheetSection("Pagina 1 - Armas y trucos de dano", [
      sheetField("ARMAS Y TRUCOS DE DANO", attackOptions(derived), "Formato de la tabla: nombre | bonif. ataque/CD | dano y tipo | notas."),
    ]),
    sheetSection("Pagina 2 - Magia", [
      sheetField("Aptitud magica", spellcastingAbility(derived), "Solo aplica si la clase o dote otorga lanzamiento de conjuros."),
      sheetField("MODIFICADOR POR APTITUD MAGICA", spellcastingModifier(derived), "Calcula con la aptitud magica correspondiente."),
      sheetField("CD DE SALVACION DE CONJUROS", spellSaveDc(derived), "8 + competencia + modificador por aptitud magica."),
    ...
    sheetSection("Pagina 2 - Equipo, monedas e idiomas", [
      ...
      sheetField("OBJETOS MAGICOS", magicItemText(derived), "Copia nombre, rareza, sintonia y efecto corto de los objetos aprobados por el DM."),
      sheetField("IDIOMAS", derived.languages.length ? displayList(derived.languages) : "Pendiente", "Copia los idiomas otorgados por la especie y futuras elecciones."),
    ]),
  ];
}
...
function attackOptions(derived) {
  ...
  if (derived.classData?.id === "monk") {
    ...
    options.push(`Golpe sin armas | ${signed(attackBonus)} / CD ${focusDc} | 1d8${signed(derived.abilityModifiers.dexterity)} contundente | Artes marciales: accion adicional.`);
  }
  ...
  weapons.forEach((weapon) => {
    ...
    const notes = [
      ...
      derived.featureEffects.criticalRange < 20 ? `critico ${derived.featureEffects.criticalRange}-20` : "",
      weapon.range ? `alcance ${formatRange(weapon.range)}` : "",
      weapon.mastery ? `maestria ${displayValue(weapon.mastery)}` : "",
      derived.classData?.id === "barbarian" && ability === "strength" ? "Furia +2 dano" : "",
...
function spellSelectionExplanation(derived) {
  ...
  if (derived.subclassData?.id === "arcane-trickster") {
    return `${preparation.sourceLabel}: prepara ${derived.spellcasting.preparedSpells.length}; al subir de nivel de Picaro puedes reemplazar 1 conjuro preparado.`;
  }

  if (derived.classData?.id === "wizard") {
    return "El Mago prepara desde el grimorio; copia tambien los conjuros escritos en GRIMORIO.";
  }
...
function magicItemSheetLine(item) {
  ...
  const attunement = item.requiresAttunement ? ", sintonia" : "";
  const hasGenericText = item.sheetText?.includes("efecto segun DMG/DM");
  const text = hasGenericText
    ? `${displayName(item)}: efecto segun manual o DM.`
    : sheetText(item);
...
function formatCastingTime(value) {
  if (value === "Action") return "accion";
  if (value === "Action or Ritual") return "accion o ritual";
  if (value === "Bonus Action") return "accion adicional";
  if (value.startsWith("Bonus Action")) return "accion adicional";
  if (value.startsWith("Action")) return "accion";
  if (value.startsWith("Reaction")) return "reaccion";
  if (value === "1 minute or Ritual") return "1 minuto o ritual";
  return value
    .replace("1 action", "accion")
    .replace("1 bonus action", "accion adicional")
    .replace("1 reaction", "reaccion");
}
