...
export function Layout({ activeRoute, activeTheme, isDarkMode, onNavigate, onDarkModeChange }) {
  ...
  sidebar.querySelector("[data-sidebar-footer]").append(ThemeSwitcher({ activeTheme }));
  const mainLayout = document.createElement("div");
  mainLayout.className = "main-layout";
  const mainHeader = document.createElement("header");
  mainHeader.className = "main-header";
  mainHeader.dataset.tour = "top-bar";
  const menuButton = IconButton({ label: "Abrir menú", icon: "menu", className: "mobile-menu-button", onClick: () => setSidebarState(true) });
  ...
  `;
  mainHeader.append(menuButton, headerBrand, HeaderActions({ isDarkMode, onDarkModeChange }));
  ...
  return { element: shell, pageRoot: main, closeSidebar: () => setSidebarState(false) };
}
